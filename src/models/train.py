...
def get_model(kind: str):
    ...
    if kind == 'xgb':
        return XGBClassifier(
            ...
            subsample=0.9, colsample_bytree=0.9, eval_metric='logloss', n_jobs=-1, random_state=42
        )
    if kind == 'lgbm':
        return LGBMClassifier(n_estimators=400, learning_rate=0.05, max_depth=-1, random_state=42, n_jobs=-1)
