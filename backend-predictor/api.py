...
def predict(item: ObraIn):
    ...
    if df.get('flag_muchas_ampliaciones') is not None and int(df['flag_muchas_ampliaciones'][0])==1:
    ...
    if df.get('flag_contratacion_directa') is not None and int(df['flag_contratacion_directa'][0])==1:
    ...
    return {
        'prob_riesgo': float(proba[0]),
        'pred_riesgo': int(pred[0]),
        'top_flags': flags
    }
